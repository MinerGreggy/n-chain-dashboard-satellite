...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'nchain-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('nchain').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('nchain')[service]);
